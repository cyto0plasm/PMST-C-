...
namespace PMST {
	 void UIHelper::ShowUserFriendlyError(Exception^ ex, String^ context) {
		SQLiteException^ sqliteEx = dynamic_cast<SQLiteException^>(ex);
		if (sqliteEx != nullptr) {
			if (sqliteEx->ErrorCode == static_cast<int>(SQLiteErrorCode::Constraint)) {
				MessageBox::Show("A unique constraint was violated. Please check for duplicates.", context, MessageBoxButtons::OK, MessageBoxIcon::Warning);
			}
			else {
				MessageBox::Show("Database error: " + sqliteEx->Message, context, MessageBoxButtons::OK, MessageBoxIcon::Error);
			}
		}
		else {
			MessageBox::Show("Unexpected error: " + ex->Message, context, MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
	}
	...
	void UIHelper::HandleCellMouseEnter(System::Object^ sender, System::Windows::Forms::DataGridViewCellEventArgs^ e)
	{
		DataGridView^ dgv = safe_cast<DataGridView^>(sender);

		if (e->RowIndex >= 0 && e->ColumnIndex >= 0) {
			DataGridViewCell^ cell = dgv->Rows[e->RowIndex]->Cells[e->ColumnIndex];
			cell->Style->BackColor = System::Drawing::Color::FromArgb(70, 70, 75);
		}
		else if (e->RowIndex == -1 && e->ColumnIndex >= 0) {
			dgv->Columns[e->ColumnIndex]->HeaderCell->Style->BackColor = System::Drawing::Color::FromArgb(75, 75, 80);
		}
	}

	void UIHelper::HandleCellMouseLeave(System::Object^ sender, System::Windows::Forms::DataGridViewCellEventArgs^ e)
	{
		DataGridView^ dgv = safe_cast<DataGridView^>(sender);
		...
		if (e->RowIndex >= 0 && e->ColumnIndex >= 0) {
			DataGridViewCell^ cell = dgv->Rows[e->RowIndex]->Cells[e->ColumnIndex];
			if (e->RowIndex % 2 == 0) {
				cell->Style->BackColor = System::Drawing::Color::FromArgb(55, 55, 58);
			}
			else {
				cell->Style->BackColor = System::Drawing::Color::FromArgb(52, 52, 55);
			}
		}
		else if (e->RowIndex == -1 && e->ColumnIndex >= 0) {
			dgv->Columns[e->ColumnIndex]->HeaderCell->Style->BackColor = System::Drawing::Color::FromArgb(60, 60, 65);
		}
	}
